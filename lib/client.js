...
Client.prototype._request = function (position, method, callback) {
  ...
  request({
    ...
    if (res.statusCode >= 400) return callback(new Error('Petición Incorrecta'))
    if (res.statusCode === 404) return callback(new Error('No hay resultados'))
    if (res.statusCode >= 500) return callback(new Error('Problema con el Servidor!'))
    else if (res.statusCode !== 200) return callback(new Error('An error ocurred in the request'))
